...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    ...
    setAuthError('جاري التحقق من صلاحيات المتجر...');
  ...
  const captureInvoice = async (mode) => {
    ...
    try {
      ...
      if (mode === 'copy' && navigator.clipboard?.write && window.ClipboardItem) {
        ...
        setToast('تم نسخ صورة الفاتورة.');
        return;
      }
      const a = document.createElement('a');
      a.href = canvas.toDataURL('image/png');
      a.download = `invoice-${selectedOrder.id}.png`;
      a.click();
      setToast('تم تحميل صورة الفاتورة.');
    } catch (error) {
  ...
  if (expandedStat) {
    ...
    return (
      <div style={{ minHeight: '100vh', background: T.bg, display: 'flex', flexDirection: 'column', fontFamily: 'Tajawal, sans-serif', direction: t.dir }}>
        {topbar(isCustomers ? t.customers : t.orders, [topBtn('رجوع', () => setExpandedStat(null))], [])}
    ...
    return (
      <div style={{ minHeight: '100vh', background: T.bg, display: 'flex', flexDirection: 'column', fontFamily: 'Tajawal, sans-serif', direction: t.dir }}>
        {topbar(
          ...
          [topBtn(t.signOut, () => signOutGoogle()), <LanguageSwitcher key="lang" lang={lang} setLang={setLang} ghost />],
          [topBtn(t.clearAll, clearAll), <img key="logo" src={logoImg} alt="" style={{ width: 32, height: 32, borderRadius: '50%', objectFit: 'cover', border: '2px solid rgba(255,255,255,0.4)' }} />],
        )}
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 14px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {sorted.map((order) => {
              ...
              return (
                <div key={order.id} onClick={() => { setSelectedOrder(order); setView('detail'); }} style={{ ...card, cursor: 'pointer', borderRight: `4px solid ${order.status === 'done' ? '#4caf50' : T.accent}` }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 6 }}>
                    <span style={badge(order.status)}>{statusLabel(order.status, t)}</span>
                    <div style={{ textAlign: 'right' }}><div style={{ fontWeight: 700, color: T.text, fontSize: 13 }}>{order.name}</div><div style={{ color: T.textMuted, fontSize: 10 }}>{formatTime(order.time, lang)}</div></div>
                  </div>
                  <div style={{ color: T.textMuted, fontSize: 10, direction: 'ltr', textAlign: 'left', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{order.link}</div>
  ...
  if (view === 'detail' && selectedOrder) {
    ...
    return (
      <div style={{ minHeight: '100vh', background: T.bg, display: 'flex', flexDirection: 'column', fontFamily: 'Tajawal, sans-serif', direction: t.dir }}>
        {topbar(t.orderDetails, [topBtn(t.invoice, () => setView('invoice')), topBtn(order.status === 'pending' ? t.markDone : t.markPending, () => toggleStatus(order))], [topBtn('رجوع', () => setView('dashboard'))])}
          ...
          <div style={card}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              <div style={{ display: 'flex', gap: 6 }}>
                <button onClick={() => startVoice(order)} style={{ padding: '6px 10px', borderRadius: 8, border: 'none', background: isRecording ? '#e05c5c' : T.accentLight, color: isRecording ? '#fff' : T.accent, fontSize: 12, cursor: 'pointer' }}>{isRecording ? 'تسجيل' : 'صوت'}</button>
                <label style={{ padding: '6px 10px', borderRadius: 8, background: T.accentLight, color: T.accent, fontSize: 12, cursor: 'pointer', display: 'inline-flex', alignItems: 'center' }}>
                  صور<input type="file" accept="image/*" multiple style={{ display: 'none' }} onChange={(e) => handleImageUpload(order, e)} />
                </label>
              </div>
  ...
  if (view === 'invoice' && selectedOrder) {
    ...
    return (
      <div style={{ minHeight: '100vh', background: T.bg, display: 'flex', flexDirection: 'column', fontFamily: 'Tajawal, sans-serif', direction: t.dir }}>
        {topbar(t.invoice, [topBtn('نسخ', () => captureInvoice('copy')), topBtn('واتساب', openWhatsApp), topBtn('طباعة', () => window.print())], [topBtn('رجوع', () => setView('detail'))])}
        {toast && <button onClick={() => setToast('')} style={{ background: '#222', color: '#fff', border: 'none', padding: 8, fontSize: 11 }}>{toast}</button>}
        <div style={{ flex: 1, overflowY: 'auto', padding: 14 }}>
          <div ref={invoiceRef} style={{ background: '#fff', borderRadius: 20, overflow: 'hidden', boxShadow: `0 4px 24px ${T.shadow}` }}>
            <div style={{ background: `linear-gradient(135deg, ${T.accent}18, ${T.accentLight})`, padding: '22px 18px', textAlign: 'center', borderBottom: `1px solid ${T.border}` }}>
              ...
              <div style={{ fontSize: 11, color: T.textMuted, marginTop: 2 }}>{lang === 'ar' ? 'التوصيل لجميع أنحاء المغرب' : 'Livraison partout au Maroc'}</div>
            </div>
            <div style={{ padding: '14px 18px', borderBottom: `1px solid ${T.border}` }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <div style={{ fontSize: 11, color: T.textMuted }}>{formatTime(order.time, lang)}</div>
                <div style={{ fontSize: 11, color: T.textMuted }}>طلب #{order.id}</div>
              </div>
            </div>
            <div style={{ padding: '0 18px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead><tr style={{ borderBottom: `2px solid ${T.border}` }}><th style={{ padding: '10px 0', fontSize: 11, color: T.textMuted, textAlign: 'left' }}>البيع</th><th style={{ padding: '10px 0', fontSize: 11, color: T.textMuted, textAlign: 'center' }}>التكلفة</th><th style={{ padding: '10px 0', fontSize: 11, color: T.textMuted, textAlign: 'right' }}>المنصة</th></tr></thead>
                <tbody>
